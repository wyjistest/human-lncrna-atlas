...
- `GET /api/v1/admin/metrics` - 系统监控指标汇总（轻量 in-memory 聚合；含 cache_stats/cache_breakdown + DB 查询耗时/慢查询（best-effort）；服务重启后清零）
...
> 提示：Prometheus 指标端点为 `GET /metrics`（默认也受 `X-Admin-API-Key` 保护，详见 `app/middleware/security/metrics_auth.py`）。
