...
interface BatchHeatmapMatrixProps {
  ...
  /** Error state */
  error?: Error | null;
  /** Whether to show the built-in metric selector */
...
function getMetricLabel(
  ...
  const labels: Record<HeatmapMetricType, string> = {
    median_fold_enrichment: t(
      "detail.chipseq.cellLineCompare.foldEnrichment",
      "Fold Enrichment",
    ),
...
export function BatchHeatmapMatrix({
  ...
  const { yLabels, heatmapData, valueRange } = useMemo(() => {
    ...
    data.forEach((geneData) => {
      ...
      const geneMarkIndices = geneData.marks.map((mark) =>
        markIndexMap.get(mark),
      );
    ...
    const range = getMinMax(validValues, {
      defaultMin: 0,
      defaultMax: 1,
      include: [0, 1],
    });
    ...
    return {
      ...
      visualMap: {
        ...
        formatter: ((value: number) =>
          formatMetricValue(value, metric)) as VisualMapFormatter,
      },
      series: [
        {
    ...
    const handleClick = (params: unknown) => {
      ...
      if (p.componentType === "series" && p.seriesType === "heatmap") {
        ...
        for (const gene of data) {
          ...
          if (
            cellIdx >= currentYIdx &&
            cellIdx < currentYIdx + gene.cell_types.length
          ) {
            geneName = gene.gene_name;
    ...
    <Space orientation="vertical" size="middle" style={{ width: "100%" }}>
      ...
      <Card size="small">
        <Row gutter={[16, 16]} align="middle">
          <Col xs={24} md={showMetricSelector ? 14 : 24}>
            <Text strong style={{ fontSize: 14 }}>
              {t("batchGeneHeatmap.matrixTitle", "Batch Gene ChIP-seq Heatmap")}
            </Text>
            <br />
            <Text type="secondary" style={{ fontSize: 12 }}>
              {data.length} genes, {allMarks.length} marks,{" "}
              {allCellTypes.length} cell types
            </Text>
          </Col>
          {showMetricSelector && (
            <Col xs={24} md={10}>
              <Segmented
                ...
                value={metric}
                onChange={(value) =>
                  onMetricChange?.(value as HeatmapMetricType)
                }
                block
              />
            </Col>
      ...
      <Card
        size="small"
        title={t("detail.chipseq.cellTypes.legend", "Cell Types")}
      >
        <Space wrap>
      ...
      <Card size="small">
        ...
        >
          <div>
            <span style={{ fontWeight: 500 }}>Total dimensions:</span>{" "}
            {data.length} genes × {allMarks.length} marks ×{" "}
            {allCellTypes.length} cell types
          </div>
          <div>
            ...
            {data.reduce(
              (sum, g) => sum + g.marks.length * g.cell_types.length,
              0,
            )}{" "}
            total
          </div>
