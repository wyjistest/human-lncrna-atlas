...
describe('paper-first narrative locale content', () => {
  it('aligns English public labels with the paper-facing narrative', () => {
    ...
    expect(enNav.snapshot).toBe('Submission Snapshot')
    ...
    expect(enDiseases.search.placeholder).toBe('Search trait name')
    expect(enRegulations.title).toBe('Candidate Regulatory Edges')
    expect(enNetwork.title).toBe('Trait-centered Networks')
    expect(enNetwork.disease.label).toBe('Trait')
    expect(enNetwork.disease.placeholder).toBe('Select trait')
    expect(enNetwork.query.selectDiseaseOntology).toBe('Please select a trait and ontology')
    ...
    expect(enAnalysis.disease.stats.totalDiseases).toBe('Traits Analyzed')
    expect(enAnalysis.disease.charts.sharedMatrix).toBe('Trait-shared lncRNA Matrix')
    expect(enAnalysis.disease.table.disease).toBe('Trait')
  ...
  it('aligns Simplified Chinese public labels with the same narrative', () => {
    ...
    expect(zhNav.snapshot).toBe('投稿快照')
    ...
    expect(zhDiseases.search.placeholder).toBe('搜索性状名称')
    expect(zhRegulations.title).toBe('候选调控边')
    expect(zhNetwork.title).toBe('性状中心网络')
    expect(zhNetwork.disease.label).toBe('性状')
    expect(zhNetwork.disease.placeholder).toBe('选择性状')
    expect(zhNetwork.query.selectDiseaseOntology).toBe('请选择性状和Ontology')
    ...
    expect(zhAnalysis.disease.stats.totalDiseases).toBe('分析性状数')
    expect(zhAnalysis.disease.charts.sharedMatrix).toBe('性状共享 lncRNA 矩阵')
    expect(zhAnalysis.disease.table.disease).toBe('性状')
