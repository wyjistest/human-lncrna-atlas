...
> 更新（2026-01-24）：本文档为历史性能事故分析与复盘快照；现状以 `docs/CURRENT_STATUS.md` 为准。
...
> 说明（更新：2026-01-29）：以下为当时的“应急实施计划”记录，用于复盘与对照，不代表当前待办；现状以 `docs/CURRENT_STATUS.md` 为准。

### Phase 1: 立即实施（今天，5 分钟）

- （当时计划）修改 API，强制要求至少一个筛选条件
- （当时计划）添加查询超时保护（30 秒）
- （当时计划）重启后端服务

### Phase 2: 性能优化（明天，0.5 天）

- （当时计划）创建物化视图 `mv_lncrna_chipseq_overlaps`
- （当时计划）为物化视图创建 7 个索引
- （当时计划）修改 API 查询，从物化视图读取
- （当时计划）测试性能提升
- （当时计划）配置自动刷新（cron）
...
- （当时计划）启用慢查询日志
- （当时计划）配置监控脚本（每 5 分钟检查）
- （当时计划）添加 Grafana 监控面板（可选）
...
> 说明：本节为“运维检查清单”；文中 `[ ]` 代表每次检查的手工勾选项，不代表仓库待办或开发计划。
