...
- `Tests` / `Security Audit` 默认只对 `main` 分支 `push` 自动触发；若你在 `.worktrees/<topic>` 或普通 feature branch 上验证改动，优先使用 `bash scripts/ci/verify_branch.sh --branch "<branch>"`（PR 可在 `gh pr checkout <PR_NUMBER>` 后使用 `--pr <PR_NUMBER>`）。该脚本会先跑一次本地门禁，并在内部 push 时自动透传 `SKIP_LOCAL_CI=1`，避免 pre-push hook 重复再跑一次。
