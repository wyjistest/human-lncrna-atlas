...
> 更新（2026-01-24）：本文档为会话总结快照，用于回溯当时的实现与结论；现状以 `docs/CURRENT_STATUS.md` 为准。
...
> 说明（更新：2026-01-29）：以下为 2025-11-27 会话结束时记录的“后续想法”，仅用于历史回溯，不代表当前待办；现状以 `docs/CURRENT_STATUS.md` 为准。

### 中优先级
- （历史想法）ESLint 规则：禁止直接引用 `@/types/api`
- （历史想法）Bundle 优化：ECharts 378KB 可进一步按需导入

### 低优先级
- （历史想法）测试：MSW 测试配置，组件单元测试
- （历史想法）文档：README 更新，API 契约文档
