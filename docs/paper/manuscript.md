...
Here we map orthologous lncRNAs and protein-coding genes across human, chimpanzee, macaque, and marmoset, and use triplex-informed inference to reconstruct candidate lncRNA regulatory edges that can be compared across primate evolution. The frozen working snapshot contains **804,630** predicted lncRNA to protein-coding gene relationships with complete per-edge sequence coverage.
