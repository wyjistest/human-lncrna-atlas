...
# ==============================================================================
# Generate epigenetic summary report using local PostgreSQL + sample DB
# ==============================================================================
#
# Purpose:
# - Quick smoke test for scripts/research/epigenetic_summary_by_binding_affinity.py
# - No need for the full production dataset
#
# What it does:
# - Creates a temporary Postgres database
# - Loads schema/v2.3 core/extension + chipseq schema + sample data + overlap MVs
# - Runs the epigenetic summary exporter against that DB
# - Drops the temporary DB (unless KEEP_DB=true)
#
# Notes:
# - The v2.3 sample dataset uses BA values around ~55-82, so MIN_BA defaults to 50.
# - MV `mv_lncrna_chipseq_overlaps_epigenetic_summary_ba100` is fixed at BA>=100, so sample
#   baseline uses the GROUP BY fallback (min_ba=50) to ensure non-empty output.
#
# Output (defaults):
# - docs/baselines/research/epigenetic-summary-ba${MIN_BA}.(tsv|md)
#
# Requirements:
...
echo "[research-baseline-local] loading schema + sample data + overlap MVs..."
for f in \
  "schema/v2.3/01_core.sql" \
  "schema/v2.3/02_extension.sql" \
  "frontend/backend/sql/chipseq_schema.sql" \
  "schema/v2.3/03_sample_data.sql" \
  "schema/v2.3/05_mv_lncrna_chipseq_overlaps.sql" \
  "schema/v2.3/06_mv_lncrna_chipseq_overlaps_epigenetic_summary_ba100.sql"; do
  echo "  - $f"
  psql -v ON_ERROR_STOP=1 -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -f "$f" >/dev/null
done

echo "[research-baseline-local] generating epigenetic summary (min_ba=$MIN_BA) ..."
mkdir -p "$OUT_DIR"
...
DB_PASSWORD="$DB_PASSWORD" \
DB_NAME="$DB_NAME" \
"$python_bin" "scripts/research/epigenetic_summary_by_binding_affinity.py" \
  --min-ba "$MIN_BA" \
