...
# ==============================================================================
# Generate Top lncRNA by Binding Affinity report using local PostgreSQL + sample DB
# ==============================================================================
#
# Purpose:
# - Quick smoke test for scripts/research/top_lncrna_by_binding_affinity.py
...
# - Runs the report generator against that DB
# - Drops the temporary DB (unless KEEP_DB=true)
#
# Notes:
# - The v2.3 sample dataset uses BA values around ~55-82, so MIN_BA defaults to 50.
#   If you set MIN_BA=100 on the sample DB, you will likely get an empty report.
#
# Output (defaults):
# - docs/baselines/research/top-lncrna-ba${MIN_BA}-species${SPECIES_ID}.(csv|md)
#
# Requirements:
...
echo "[research-baseline-local] generating report (species_id=$SPECIES_ID, min_ba=$MIN_BA, limit=$LIMIT) ..."
mkdir -p "$OUT_DIR"
...
DB_PASSWORD="$DB_PASSWORD" \
DB_NAME="$DB_NAME" \
"$python_bin" "scripts/research/top_lncrna_by_binding_affinity.py" \
