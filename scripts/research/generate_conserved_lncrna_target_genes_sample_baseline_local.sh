...
# ==============================================================================
# Generate conserved lncRNA target genes report using local PostgreSQL + sample DB
# ==============================================================================
#
# Purpose:
# - Quick smoke test for scripts/research/conserved_lncrna_target_genes_for_enrichment.py
...
# - Runs the conserved target genes exporter against that DB
# - Drops the temporary DB (unless KEEP_DB=true)
#
# Notes:
# - The v2.3 sample dataset uses BA values around ~55-82, so MIN_BA defaults to 50.
# - Sample data includes lncRNA rows only for human (species_id=1).
#   For SPECIES_IDS=all, conservation level species_count==4 will be empty on the sample DB.
#   To keep the smoke test meaningful, INCLUDE_SPECIFIC defaults to true (also exports species_count==1).
#
# Output (defaults):
# - docs/baselines/research/conserved-lncrna-target-genes-ba${MIN_BA}-sc*.{tsv,txt,md}
#
# Requirements:
...
MIN_BA="${MIN_BA:-50}"
SPECIES_COUNT="${SPECIES_COUNT:-4}"
INCLUDE_SPECIFIC="${INCLUDE_SPECIFIC:-true}"
...
echo "[research-baseline-local] generating conserved target genes (species_ids=$SPECIES_IDS, min_ba=$MIN_BA, species_count=$SPECIES_COUNT) ..."
mkdir -p "$OUT_DIR"

extra_args=()
if [ "$INCLUDE_SPECIFIC" = "true" ]; then
  extra_args+=("--include-specific")
fi
if [ "$TARGET_PROTEIN_CODING_ONLY" = "true" ]; then
  extra_args+=("--target-protein-coding-only")
fi
...
DB_PASSWORD="$DB_PASSWORD" \
DB_NAME="$DB_NAME" \
"$python_bin" "scripts/research/conserved_lncrna_target_genes_for_enrichment.py" \
