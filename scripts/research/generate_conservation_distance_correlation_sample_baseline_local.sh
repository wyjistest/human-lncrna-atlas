...
# ==============================================================================
# Generate conservation-distance correlation report using local PostgreSQL + sample DB
# ==============================================================================
#
# Purpose:
# - Quick smoke test for scripts/research/conservation_distance_correlation_by_binding_affinity.py
...
# - Runs the conservation-distance exporter against that DB
# - Drops the temporary DB (unless KEEP_DB=true)
#
# Notes:
# - The v2.3 sample dataset uses BA values around ~55-82, so MIN_BA defaults to 50.
# - Sample data includes lncRNA rows only for human (species_id=1). Other species may yield 0, so correlations can be N/A.
#
# Output (defaults):
# - docs/baselines/research/conservation-distance-ba${MIN_BA}-species-${SPECIES_IDS}*.{csv,md,png}
#
# Requirements:
...
echo "[research-baseline-local] generating conservation-distance report (species_ids=$SPECIES_IDS, min_ba=$MIN_BA) ..."
mkdir -p "$OUT_DIR"
...
DB_PASSWORD="$DB_PASSWORD" \
DB_NAME="$DB_NAME" \
"$python_bin" "scripts/research/conservation_distance_correlation_by_binding_affinity.py" \
