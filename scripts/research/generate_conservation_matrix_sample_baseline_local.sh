...
# ==============================================================================
# Generate conservation matrix report using local PostgreSQL + sample DB
# ==============================================================================
#
# Purpose:
# - Quick smoke test for scripts/research/conservation_matrix_by_binding_affinity.py
...
# - Runs the conservation matrix exporter against that DB
# - Drops the temporary DB (unless KEEP_DB=true)
#
# Notes:
# - The v2.3 sample dataset uses BA values around ~55-82, so MIN_BA defaults to 50.
# - Sample data includes lncRNA rows only for human (species_id=1). Other species may yield 0 in the matrix.
#
# Output (defaults):
# - docs/baselines/research/conservation-matrix-ba${MIN_BA}-species-${SPECIES_IDS}-*.{csv,md,png}
#
# Requirements:
...
echo "[research-baseline-local] generating conservation matrix (species_ids=$SPECIES_IDS, min_ba=$MIN_BA) ..."
mkdir -p "$OUT_DIR"
